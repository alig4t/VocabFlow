...
| شرط | نوع | تیکِ کنترل‌کننده | لیبلِ تنظیمات |
|---|---|---|---|
| روز ≥ ۳ (غیبت طولانی) | `overdue` | `notifyOverdue` | یادآور مرورهای عقب‌افتاده |
| روز ۰ و `streak > 0` | `streak` | `notifyStreak` | **یادآور حفظ پشتکار** |
| در غیر این صورت | `daily` | `notifyDailyStudy` | یادآور مطالعه‌ی روزانه |
...
**`streak` — روز ۰، پشتکارِ فعال:**
> 🔥 پشتکار ‪N‬ روزه‌ات در خطره
> امروز هنوز مطالعه نکرده‌ای؛ یه مرور کوتاه کافیه.

**`overdue` — از روز ۳ غیبت به بعد:**
> 🔥 چند روزه ندیدیمت
> لغت‌هات منتظر مرورن؛ چند دقیقه وقت بذار و پشتکارت رو دوباره بساز.

### ⚠️ واژه‌نامه: `streak` در کد، **«پشتکار»** برای کاربر
داشبورد از قبل همین عدد را با لیبلِ **«پشتکار»** (آیکون 🔥، مقدار `۴ روز`، hint «روزهای متوالی») نشان می‌دهد
— `components/dashboard/GlobalStats.tsx`. پس **کلمه‌ی «استریک» نباید در هیچ متنِ کاربر ظاهر شود**؛
کاربر لازم نیست دو اسم برای یک مفهوم یاد بگیرد. `streak` فقط اسمِ فیلد/انتیتی در کد است.
