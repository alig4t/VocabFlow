...
> برنامه‌ی اعلان‌ها **هر بار از صفر بازسازی می‌شود** و یک **نردبانِ کاهشی** جلو زمان‌بندی می‌گردد.
...
### نردبانِ کاهشی (`REMINDER_DAYS`) — کم‌آزار و ملایم
```ts
const REMINDER_DAYS = [0, 1, 3, 6, 10, 15, 21, 30]
```
به‌جای «هر روز یک پیام»، هرچه غیبت طولانی‌تر شود **فاصله‌ها بازتر می‌شوند**: مجموعاً **۸ پیام در ۳۰ روز**.

| روز | ۰ | ۱ | ۳ | ۶ | ۱۰ | ۱۵ | ۲۱ | ۳۰ |
|---|---|---|---|---|---|---|---|---|
| نوع | daily | daily | overdue | overdue | overdue | overdue | overdue | overdue |
| فاصله از قبلی | — | ۱ | ۲ | ۳ | ۴ | ۵ | ۶ | ۹ |

**چرا روز ۰ و ۱ هر دو لازم‌اند؟** لنگرِ زمان‌بندی «آخرین باز کردن اپ» است، نه «شروع غیبت»:
- **روز ۰** = امروز، هنوز مطالعه نکرده‌ای (یادآورِ اصلی).
- **روز ۱** = فردا، یعنی «کلاً اپ را باز نکردی». اگر این را برداریم، کاربری که فقط **یک روز** جا می‌اندازد **هیچ یادآوری نمی‌گیرد** — دقیقاً همان لحظه‌ای که بیشترین اهمیت را دارد.

از روز ۳ به بعد لحن به «برگشت» تغییر می‌کند و متن‌ها **چرخشی** (۳ نسخه) هستند تا تکراری حس نشود.

> **این طراحی دو باگِ نسخه‌ی اول را حل کرد:**
> ۱. *تکرارِ آزاردهنده* — قبلاً روزهای ۳ تا ۶ عیناً یک متن را ۴ بار می‌فرستادند (خلافِ اصلِ
>    «limit the number of notifications» در spec).
> ۲. *پرتگاهِ سکوت* — چون بازسازی فقط با باز کردن اپ رخ می‌دهد، افقِ ۷ روزه یعنی کاربرِ غایب بعد از روز ۶
>    **برای همیشه** هیچ پیامی نمی‌گرفت؛ یعنی همان کسی که بیشتر از همه به برگشتن نیاز داشت رها می‌شد.
>    نردبان با **پیامِ کمتر**، پوششِ **خیلی طولانی‌تری** می‌دهد.
...
### شرط روزهای آینده (بقیه‌ی پله‌های نردبان)
...
**`overdue` — از روز ۳ به بعد، چرخشی روی پله‌های نردبان** (`OVERDUE_MESSAGES`، لحنِ کم‌فشار):

| پله | روزها | متن |
|---|---|---|
| ۱ | ۳، ۱۵ | 📚 چند روزه ندیدیمت — هر وقت فرصت داشتی، لغت‌هات همین‌جا منتظرتن. |
| ۲ | ۶، ۲۱ | 🌱 یه مرور کوتاه کافیه — چند دقیقه وقت بذار و پشتکارت رو دوباره بساز. |
| ۳ | ۱۰، ۳۰ | ✨ هر وقت آماده بودی، از همون‌جا ادامه بده — شروع دوباره سخت نیست؛ از همون لغت بعدی. |

چرخش با `OVERDUE_DAYS.indexOf(day) % OVERDUE_MESSAGES.length` انجام می‌شود؛ پس هیچ‌وقت دو پیامِ پشت‌سرهم یکسان نیستند.
...
- ثابت‌ها در `src/lib/notifications.ts`:
  - `REMINDER_DAYS = [0,1,3,6,10,15,21,30]` — نردبانِ کاهشی (۸ پیام در ۳۰ روز)
