...
export const vocabularyService = {
  ...
  getWord(id: string): Promise<Word> {
    ...
    return api
      .get<Word>(API_ENDPOINTS.words.detail(id))
      .then((res) => res.data);
  },

  getModules(): Promise<LearningModule[]> {
    if (isNative())
      return off().then((o) => o.getModules() as Promise<LearningModule[]>);
    return api
      .get<LearningModule[]>(API_ENDPOINTS.words.modules)
      .then((res) => res.data);
  },
  ...
  createWord(data: CreateWordData): Promise<Word> {
    if (isNative())
      return off().then((o) =>
        o.createWord(data as unknown as Record<string, unknown>),
      );
    return api
      .post<Word>(API_ENDPOINTS.words.list, data)
      .then((res) => res.data);
  },
  ...
  updateWord(id: string, data: UpdateWordData): Promise<Word> {
    if (isNative())
      return off().then((o) =>
        o.updateWord(id, data as unknown as Record<string, unknown>),
      );
    return api
      .put<Word>(API_ENDPOINTS.words.detail(id), data)
      .then((res) => res.data);
  },
  ...
  updateExample(
    ...
    if (isNative())
      return off().then((o) => o.updateExample(wordId, exampleId, data));
    return api
  ...
  deleteExample(wordId: string, exampleId: string): Promise<void> {
    if (isNative())
      return off().then((o) => o.deleteExample(wordId, exampleId));
    return api
