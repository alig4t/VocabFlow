...
const COVER_BY_TITLE: Record<string, string> = {
  ...
  "1000 English Collocations in 10 Minutes a Day":
    "/books/1000-collocation.png",
  "English Phrasal Verbs in Use": "/books/english-phrasal-verbs-in-use.png",
  "Barron's Essential Words for the GRE":
    "/books/Essentaial-words-for-the-GRE-1.png",
  "Barron's Essential Words for the IELTS":
    "/books/Essentaial-words-for-the-ielts.png",
  "Barron's Essential Words for the TOEFL":
    "/books/Essentaial-words-for-the-toefl.png",
  "Street Talk 1": "/books/street-talk.png",
  "504 Absolutely Essential Words":
    "/books/504-absolutely-essential-words-v1.webp",
  "Barron's 1100 Words You Need to Know":
    "/books/barron-1100-words-you-need-to-know.webp",
  "English Collocations in Use":
    "/books/ENGLISH-collocations-IN-USE-collection.webp",
...
function buildWhere(f: WordFilters): { clause: string; params: unknown[] } {
  ...
  if (f.lessonId !== undefined) {
    parts.push("w.lesson_id = ?");
    params.push(f.lessonId);
  }
  if (f.volumeId !== undefined) {
    parts.push("l.volume_id = ?");
    params.push(f.volumeId);
  }
  if (f.bookId !== undefined) {
    parts.push("v.book_id = ?");
    params.push(f.bookId);
  }
...
async function loadExamples(
  ...
  for (const r of rows) {
    ...
    list.push({
      id: r.id,
      wordId: r.word_id,
      engSentence: r.eng_sentence,
      perTranslation: r.per_translation,
      order: r.ord,
    });
...
async function loadPhrases(
  wordIds: string[],
): Promise<Map<string, Word["phrases"]>> {
  ...
  if (phraseIds.length > 0) {
    ...
    for (const e of exs) {
      ...
      l.push({
        id: e.id,
        phraseId: e.phrase_id,
        engSentence: e.eng_sentence,
        perTranslation: e.per_translation,
        order: e.ord,
      });
...
async function loadProgress(
  wordIds: string[],
): Promise<Map<string, Word["progress"]>> {
...
export async function resetProgress(reviewMode?: ReviewMode): Promise<void> {
  if (reviewMode)
    await run(
      `UPDATE progress SET manual_status = 'NOT_READ' WHERE review_mode = ?`,
      [reviewMode],
    );
...
export async function getStats(): Promise<ProgressStats> {
  ...
  const mk = (mode: ReviewMode) => {
    const known =
      counts.find((x) => x.review_mode === mode && x.manual_status === "KNOWN")
        ?.c ?? 0;
    const notKnown =
      counts.find(
        (x) => x.review_mode === mode && x.manual_status === "NOT_KNOWN",
      )?.c ?? 0;
    return {
      KNOWN: known,
      NOT_KNOWN: notKnown,
      NOT_READ: total - known - notKnown,
      total,
    };
  };
...
export async function getBooksSimple(): Promise<BookSimple[]> {
  return query<BookSimple>(
    "SELECT id, title FROM books ORDER BY title ASC",
    [],
  );
}
export async function getVolumesSimple(
  bookId: string,
): Promise<VolumeSimple[]> {
  const rows = await query<{
    id: string;
...
export async function getLessonsSimple(
  volumeId: string,
...
export async function addToWatchlist(
  ...
  await run(
    "INSERT OR IGNORE INTO watchlist (book_id, created_at) VALUES (?, ?)",
    [bookId, now()],
  );
  return { bookId };
}
...
export async function updateWord(
  ...
  vals.push(id);
  if (sets.length > 1)
    await run(`UPDATE words SET ${sets.join(", ")} WHERE id = ?`, vals);
  return getWord(id);
}
...
export async function deleteWord(id: string): Promise<void> {
  await run(
    "DELETE FROM word_phrase_examples WHERE phrase_id IN (SELECT id FROM word_phrases WHERE word_id = ?)",
    [id],
  );
...
export async function updateExample(
  ...
  const sets: string[] = [];
  const vals: unknown[] = [];
  if (data.engSentence !== undefined) {
    sets.push("eng_sentence = ?");
    vals.push(data.engSentence);
  }
  if (data.perTranslation !== undefined) {
    sets.push("per_translation = ?");
    vals.push(data.perTranslation);
  }
  if (sets.length) {
    vals.push(exampleId);
    await run(`UPDATE word_examples SET ${sets.join(", ")} WHERE id = ?`, vals);
  }
  const rows = await query<{
    id: string;
    word_id: string;
...
export async function getSynonyms(wordId: string): Promise<SynonymResult[]> {
  const rows = await query<{ synonyms: string | null }>(
    "SELECT synonyms FROM words WHERE id = ?",
    [wordId],
  );
  return parseArr(rows[0]?.synonyms ?? null).map((w) => ({
    word: w,
    similarity: 1,
    source: "local",
  }));
...
const dayOf = (iso: string | null | undefined) =>
  iso ? isoDayLocal(new Date(iso)) : null;
 ...
 */
async function sessionTotalsBetween(
  startIso: string,
  endIso: string,
): Promise<DailyStudyTotals> {
  const rows = await query<{
    duration_sec: number;
...
export async function getDashboard(): Promise<DashboardData> {
  ...
  const sessionDays = new Set(
    sessions.map((s) => dayOf(s.started_at)).filter(Boolean) as string[],
  );
  let currentStreak = 0;
  const cursor = startOfDay(new Date());
  if (!sessionDays.has(isoDayLocal(cursor)))
...
async function queryHardWords(
  mode: ReviewMode,
  limit: number,
): Promise<HardWordItem[]> {
  const rows = await query<{
    id: string;
    eng: string;
    per: string;
...
async function getMemoryInsights(
  ...
  for (const row of dueRows) {
    ...
    const offset =
      Math.floor((due.getTime() - dayEnd.getTime()) / 86_400_000) + 1;
...
export async function getLearningStats(): Promise<LearningStats> {
  ...
  for (const s of sessions) {
    ...
    if (key)
      minutesPerDay.set(
        key,
        (minutesPerDay.get(key) ?? 0) + s.duration_sec / 60,
      );
  ...
  const accuracy =
    events.length > 0 ? Math.round((correct / events.length) * 100) : 0;

  const byMode: ModeAccuracy[] = (["EN_TO_FA", "FA_TO_EN"] as ReviewMode[]).map(
    (m) => {
      ...
      return {
        ...
        accuracy:
          row && row.reviews > 0
            ? Math.round((row.correct / row.reviews) * 100)
            : 0,
      };
    },
  );

  const daily: DailyStat[] = recentDayKeys(dayStart, STATS_DAILY_DAYS).map(
    (date) => ({
      date,
  ...
  const heatmap: HeatmapDay[] = recentDayKeys(dayStart, STATS_HEATMAP_DAYS).map(
    (date) => ({
      date,
...
function buildGrowthFromEvents(
  ...
  let i = 0;
  while (i < scoped.length && new Date(scoped[i].reviewed_at) < windowStart)
    apply(scoped[i++]);
  ...
  for (let d = 0; d < keys.length; d++) {
    ...
    while (i < scoped.length && new Date(scoped[i].reviewed_at) < boundary)
      apply(scoped[i++]);
...
function buildRecords(
  ...
  for (let i = 0; i < yearKeys.length; i++) {
    ...
    if (window > 0 && (!bestWeekReviews || window > bestWeekReviews.count)) {
      bestWeekReviews = {
        weekStart: yearKeys[Math.max(0, i - 6)],
        count: window,
      };
    }
  }
...
function assertDailyGoal(dailyNewWords: number, dailyGoal: number) {
  if (dailyGoal < dailyNewWords)
    throw new Error("هدف روزانه نمی‌تواند از واژگان جدید روزانه کمتر باشد");
  if (dailyGoal < MIN_DAILY_GOAL || dailyGoal > MAX_DAILY_GOAL) {
    throw new Error(
      `هدف روزانه باید بین ${MIN_DAILY_GOAL} و ${MAX_DAILY_GOAL} باشد`,
    );
  }
}

/** Sum of dailyNewWords across active plans, excluding one volume (being replaced/updated). */
async function sumActiveDailyNewWordsExcluding(
  volumeId: string,
): Promise<number> {
...
export async function createPlan(input: {
  ...
  if (!ALLOWED_DAILY_NEW.includes(input.dailyNewWords)) {
    throw new Error(
      `واژگان جدید روزانه باید یکی از این مقادیر باشد: ${ALLOWED_DAILY_NEW.join(", ")}`,
    );
  }
  ...
  const existing = (
    await query<{ id: string }>(
      "SELECT id FROM learning_plans WHERE volume_id=?",
      [input.volumeId],
    )
  )[0];
  if (existing) {
...
export async function updatePlan(
  ...
  if (
    input.dailyNewWords !== undefined &&
    !ALLOWED_DAILY_NEW.includes(input.dailyNewWords)
  ) {
    throw new Error(
      `واژگان جدید روزانه باید یکی از این مقادیر باشد: ${ALLOWED_DAILY_NEW.join(", ")}`,
    );
  ...
  const sets: string[] = [];
  const vals: unknown[] = [];
  if (input.dailyNewWords !== undefined) {
    sets.push("daily_new_words=?");
    vals.push(input.dailyNewWords);
  }
  if (input.dailyGoal !== undefined) {
    sets.push("daily_goal=?");
    vals.push(input.dailyGoal);
  }
  ...
  if (sets.length) {
    vals.push(id);
    await run(`UPDATE learning_plans SET ${sets.join(", ")} WHERE id=?`, vals);
  }
  return { id };
}
...
export async function deletePlan(id: string): Promise<{ id: string }> {
  const row = (
    await query<{ volume_id: string }>(
      "SELECT volume_id FROM learning_plans WHERE id=?",
      [id],
    )
  )[0];
  if (row) {
...
export async function getSettings(): Promise<UserSettings> {
  ...
  if (!row) {
    await run("INSERT OR IGNORE INTO user_settings (id) VALUES (?)", [
      SETTINGS_ID,
    ]);
    return {
...
export async function updateSettings(
  ...
  if (sets.length) {
    vals.push(SETTINGS_ID);
    await run(`UPDATE user_settings SET ${sets.join(", ")} WHERE id=?`, vals);
  }
  return getSettings();
}
...
export async function getNotificationStatus(): Promise<NotificationStatus> {
  ...
  const sessionDays = new Set(
    (
      await query<{ started_at: string | null }>(
