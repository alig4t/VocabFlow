...
interface OldFormat {
  volume: number;
  units: { unit: number; words: ScrapedWord[] }[];
}
interface NewFormat {
  bookSlug: string;
  volumeNumber: number;
...
function collectFile(
  ...
  const volumeId = uid();
  const volumeTitle =
    VOLUME_TITLE_MAP[bookSlug]?.[volumeNumber] ?? `Volume ${volumeNumber}`;
  ...
  for (const les of lessons) {
    ...
    for (const w of les.words) {
      const per = w.meanings
        .map((m) => m.per)
        .filter(Boolean)
        .join(" / ");
      const description = joinDescriptions(
        w.meanings.map((m) => m.description?.eng),
      );
      const descriptionPer = joinDescriptions(
        w.meanings.map((m) => m.description?.per),
      );
      const synonyms = [
        ...new Set(w.meanings.flatMap((m) => m.synonyms ?? [])),
      ];
      const antonyms = [
        ...new Set(w.meanings.flatMap((m) => m.antonyms ?? [])),
      ];
      ...
      const hasAnyExample =
        ...
        allPhrases.some((p) => (p.examples?.length ?? 0) > 0);
      const fallbackPhrase = !hasAnyExample ? allPhrases[0] : undefined;
      const firstEx =
        allExamples[0] ??
        (fallbackPhrase
          ? {
              eng: fallbackPhrase.patternEng,
              per: fallbackPhrase.patternPer ?? "",
            }
          : undefined);
      const wordId = uid();
      const now = NOW();
