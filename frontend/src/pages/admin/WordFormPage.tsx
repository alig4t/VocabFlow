...
import {
  ExampleManager,
  type DraftExample,
} from "@/components/admin/ExampleManager";
import {
  PhraseManager,
  type DraftPhrase,
} from "@/components/admin/PhraseManager";
...
export function WordFormPage() {
  ...
  const [selectedVolumeId, setSelectedVolumeId] =
    useState<string>(prefilledVolumeId);
  const [selectedLessonId, setSelectedLessonId] =
    useState<string>(prefilledLessonId);
  ...
  async function onSubmit(raw: WordFormInput) {
    ...
    try {
      ...
      const message =
        err?.response?.data?.message ?? err?.message ?? "خطا در ذخیره واژه";
      setSubmitError(message);
      toast({
        title: "خطا در ذخیره واژه",
        description: message,
        variant: "destructive",
      });
    }
    ...
    <div className="max-w-2xl mx-auto space-y-6 pb-12 font-persian" dir="rtl">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold">
          {isEditMode ? "ویرایش واژه" : "افزودن واژه"}
        </h1>
      </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                ...
                <Input
                  id="description"
                  placeholder="obey or do what sth states"
                  dir="ltr"
            ...
            <div className="space-y-1.5">
              ...
              <Input
                id="wordForms"
                placeholder="مثال: تفضیلی: more afraid  عالی: most afraid"
                dir="ltr"
                {...register("wordForms")}
              />
            </div>
            ...
            {isLessonPrefilled ? (
              <div className="rounded-lg border border-primary/30 bg-primary/5 px-4 py-3 space-y-2">
                <p className="text-xs font-medium text-primary">
                  این واژه به درس زیر اضافه می‌شود:
                </p>
                <div className="flex flex-wrap gap-2 text-sm font-medium text-foreground">
                  ...
                  {selectedVolume && (
                    <span className="inline-flex items-center gap-1 rounded-full bg-background border border-border px-2.5 py-0.5 text-xs">
                      📖 جلد {selectedVolume.volumeNumber}
                      {selectedVolume.title ? ` — ${selectedVolume.title}` : ""}
                    </span>
                  )}
                  {selectedLesson && (
                    <span className="inline-flex items-center gap-1 rounded-full bg-primary/10 border border-primary/30 px-2.5 py-0.5 text-xs text-primary font-semibold">
                      درس {selectedLesson.lessonNumber}
              ...
              <>
                ...
                {books && books.length > 0 ? (
                  ...
                  <p className="text-sm text-muted-foreground italic">
                    هنوز کتابی تعریف نشده است. ابتدا از بخش «کتاب‌ها» یک کتاب
                    اضافه کنید.
                  </p>
                )}
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-1.5">
              ...
              <Input
                id="primaryExample"
                placeholder="She is an eloquent speaker."
                dir="ltr"
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="او یک سخنران سخنور است."
                dir="rtl"
