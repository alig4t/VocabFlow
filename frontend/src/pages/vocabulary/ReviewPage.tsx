...
export function ReviewPage() {
  ...
  const [reviewFilter, setReviewFilter] = useState<ReviewFilter>(() =>
    searchParams.has("status")
      ? statusToReviewFilter(initial.filters.status)
      : loadPersistedFilter(),
  );
  ...
  const [session, setSession] = useState<{ key: string; words: Word[] }>({
    key: "",
    words: [],
  });
  ...
  useEffect(() => {
    if (hasScope && session.key !== sessionKey && data?.data && !isFetching) {
      ...
      setCurrentIndex(
        Math.min(Math.max(0, saved), Math.max(0, data.data.length - 1)),
      );
      setFlipped(false);
    }
  ...
  useEffect(() => {
    try {
      localStorage.setItem(
        "vocab_review_scope",
        JSON.stringify({ bookId, volumeId, lessonId }),
      );
    } catch {
      // ignore
    }
  ...
  return (
    <div
      dir="rtl"
      className="font-persian max-w-4xl mx-auto px-2 sm:px-4 py-4 sm:py-6 space-y-4"
    >
      ...
      <div className="flex items-start gap-3 rounded-xl border border-blue-200 bg-blue-50/70 p-3 dark:border-blue-900/50 dark:bg-blue-950/30">
        ...
        <div className="min-w-0 space-y-0.5">
          <p className="text-sm font-bold text-blue-900 dark:text-blue-200">
            مرور آزاد — مسیری جدا از برنامه
          </p>
          <p className="text-xs leading-relaxed text-blue-800/90 dark:text-blue-300/90">
            اینجا فقط واژه‌ها را «بلدم / بلد نیستم» علامت می‌زنید؛ این کار روی
            زمان‌بندی «مطالعه امروز» تأثیری ندارد. مناسب کسانی که می‌خواهند
            سریع‌تر و بدون برنامه‌ی مرور پیش بروند.
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-border bg-card px-6 py-14 text-center space-y-3">
          <p className="text-lg font-semibold text-foreground">
            لیست یادگیری شما خالی است
          </p>
          <p className="text-sm text-muted-foreground">
            برای شروع مرور، ابتدا از کتابخانه یک یا چند کتاب به لیست یادگیری‌تان
            اضافه کنید.
          </p>
          <Button
            variant="outline"
