...
export function useUpdateBook() {
  ...
  return useMutation<
    Book,
    Error,
    { id: string; data: Partial<CreateBookData> }
  >({
  ...
  return useMutation<Volume, Error, { bookId: string; data: CreateVolumeData }>(
    {
      ...
      onSuccess: (_v, { bookId }) =>
        queryClient.invalidateQueries({ queryKey: ["volumes", bookId] }),
    },
  );
}
...
export function useDeleteVolume() {
  ...
  return useMutation<void, Error, { bookId: string; volumeId: string }>({
    mutationFn: ({ bookId, volumeId }) =>
      bookService.deleteVolume(bookId, volumeId),
    onSuccess: (_v, { bookId }) =>
      queryClient.invalidateQueries({ queryKey: ["volumes", bookId] }),
  });
}
...
export function useCreateLesson() {
  ...
  return useMutation<
    Lesson,
    Error,
    { bookId: string; volumeId: string; data: CreateLessonData }
  >({
    mutationFn: ({ bookId, volumeId, data }) =>
...
export function useDeleteLesson() {
  ...
  return useMutation<
    void,
    Error,
    { bookId: string; volumeId: string; lessonId: string }
  >({
